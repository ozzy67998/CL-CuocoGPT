...
def _process_query(user_query: str) -> str:
    """
    Process a query and return ONLY the LLM-generated summary text.
    ...
    # Extract intent + slots (still needed for building the LLM prompt)
    intent = result.get("intent") or "(unknown)"
    intent_conf = (
        result.get("intent_confidence")
        or result.get("intent_score")
        or (result.get("intent_scores") or [None])[0]
    )
    slots = result.get("slots") or {}
    print("\n========== QUERY INFO ==========")
    ...
    if ing:
        slotq.append("ingredients: " + ", ".join(ing))
    if tg:
        slotq.append("tags: " + ", ".join(tg))
    if nm:
        slotq.append("names: " + ", ".join(nm))

    sim_query = f"{user_query} | " + " | ".join(slotq) if slotq else user_query

    # RAG similarity search (still needed for good quality answers)
    try:
        from RAG.description_index import similarity_search
